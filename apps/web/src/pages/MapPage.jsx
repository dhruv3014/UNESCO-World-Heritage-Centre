...
import { useRecords } from "@/hooks/api.js";
...
export default function MapPage() {
  ...
  const { data: sites, isLoading } = useRecords("sites", { page: 1, limit: 200, sort: "s_id", order: "asc" });
  const { data: dangerSites } = useRecords("danger-sites", { page: 1, limit: 200, sort: "s_id", order: "asc" });

  const dangerIds = useMemo(() => new Set((dangerSites?.data ?? []).map((row) => row.s_id)), [dangerSites]);
  ...
  useEffect(() => {
    ...
    if (!map || !sites) return;
    map.invalidateSize(); // the container may have been hidden while loading
      ...
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          ...
          <span className="ml-2 inline-flex items-center gap-1">
            <span className="inline-block h-3 w-3 rounded-full bg-sky-600" /> normal
          </span>
          <span className="ml-2 inline-flex items-center gap-1">
            <span className="inline-block h-3 w-3 rounded-full bg-red-600" /> in danger
          </span>
        </p>
      </div>
