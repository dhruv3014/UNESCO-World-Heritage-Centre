// Central registry describing every domain table exactly once. This single
// source of truth powers the generic CRUD/search/filter API and the frontend's
// dynamic tables, filters and forms (delivered via GET /api/meta).
//
// Field shape: { name, label, type, searchable?, filterable?, isId? }
//   type       — "string" | "number" | "boolean" | "date"
//   searchable — included in the free-text ?search= query
//   filterable — can be used in a per-field filter
...
export const RESOURCES = [
  ...
  },
  {
    ...
    primaryKey: ["year", "country_code", "category"],
    defaultSort: "year",
    fields: [
...
const byTable = Object.fromEntries(RESOURCES.map((resource) => [resource.table, resource]));

export const getResourceByKey = (key) => byKey[key];
export const getResourceByTable = (table) => byTable[table];
