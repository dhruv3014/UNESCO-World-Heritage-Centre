...
import { RESOURCES } from "../resources/resource.registry.js";

/**
 * GET /api/stats — aggregate numbers and chart-friendly breakdowns for the
 * dashboard. All aggregates ignore soft-deleted rows (deleted_at IS NULL).
 */
export async function getStats(_req, res) {
  ...
  const dangerSites = await queryOne(
    `SELECT COUNT(*)::int AS count FROM provisional_danger_site WHERE deleted_at IS NULL`,
  );
  ...
  res.json({
    ...
    sitesByCategory,
    sitesByRegion,
    fundsByType: fundsByType.map((row) => ({ type: row.type, count: row.count, total: Number(row.total) })),
    donationsOverTime: donationsOverTime.map((row) => ({ month: row.month, total: Number(row.total), count: row.count })),
