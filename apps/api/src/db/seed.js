/**
 * Loads demo users and realistic sample data.
 *
 *   npm run db:seed
 *
...
async function seedUsers() {
  ...
  await insert(
    "app_user",
    {
      email: "user@whc.org",
      name: "Public Viewer",
      password_hash: await bcrypt.hash("User@12345", 10),
      role: "USER",
    },
    "email",
  );
...
async function seedDomain() {
  ...
  for (const donor of donors) await insert("donor_detail", donor, "donor_id");
  ...
  for (const country of countries) await insert("member_country", country, "country_code");
  ...
  for (const institute of institutes) await insert("local_institute_agency", institute, "institute_id");
  ...
  for (const site of sites) await insert("site_detail", site, "s_id");
  ...
  for (const manager of managers) await insert("site_manager", manager, "m_id");
  ...
  for (const report of reports) await insert("status_report", report, "report_id");
  ...
  for (const fund of funds) await insert("fund", fund, "f_id");
  await insert("other_fund", { f_id: 1, m_id: 1 }, "f_id");
  await insert("danger_site_fund", { f_id: 3, s_id: 4 }, "f_id");

  // Spread donations across many months so the "donations over time" chart is meaningful.
  const donations = [
  ...
  for (const donation of donations) await insert("donation", donation, "transaction_id");
  ...
  for (const member of committee) await insert("world_heritage_committee", member, "member_id");
