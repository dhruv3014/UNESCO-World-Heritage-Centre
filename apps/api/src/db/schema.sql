...
--
-- Running this file DROPS and recreates everything, so it doubles as a clean
-- initializer. Every domain table has a `deleted_at` column for soft deletes:
-- deleting a record only stamps this column, so nothing is ever truly lost.
...
-- Version-control backbone: every admin write records a full before/after snapshot.
...
-- Records schema changes made through the schema editor.
